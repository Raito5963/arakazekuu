...
export default function NewsPage() {
  ...
  // id降順でソート
  const sortedNews = [...publishedNews].sort((a, b) => b.id - a.id);

  React.useEffect(() => {
    if (selectedNews) {
      // オーバーレイ表示時はbodyのスクロールを禁止
              ...
              {(() => {
                ...
                while (i < blocks.length) {
                  if (blocks[i].type === 'image') {
                    ...
                    result.push(
                      <p key={i} style={{marginBottom: '1rem'}}>{blocks[i].value}</p>
                    );
                    i++;
                  }
                }
